...
function loadSettings() {
  ...
  let body = div.find('#modal-body');
  let html = '<h2>Logout</h2>';
  html += '<p><input type="button" id="logout-this" class="modal-button-small" value="Logout this session"></p>';
  html += '<p><input type="button" id="logout-all" class="modal-button-small" value="Logout all sessions"> <span id="token-list">🛈</span></p>';
  html += '<h2>Password</h2><p><form>';
  if (app.password) html += '<div><span class="settings-label">Current password:</span><input type="password" class="input-password" name="old" autocomplete="current-password"></div>';
  html += '<div><span class="settings-label">New password:</span><input type="password" class="input-password" name="new1" autocomplete="new-password"></div>';
  html += '<div><span class="settings-label">Repeat new:</span><input type="password" class="input-password" name="new2" autocomplete="new-password"></div>';
  html += '</form></p>';
  html += '<h2>Security keys</h2>';
  html += '<ul id="list-u2f">Loading...</ul>';
  html += '<p><input type="button" id="register-u2f" class="modal-button-small" value="Register new U2F key"></p>';
  html += '<h2>Automatic snapshots</h2>';
  html += '<p><input type="checkbox" id="autosnap"> When editing, auto-snapshot every <input id="snapafter" class="input-smallint" type="number" min="1"> hours</p>';
  html += '<p><input type="checkbox" id="autoprune"> Prune automatic snapshots after <input id="pruneafter" class="input-smallint" type="number" min="1"> days, keeping<br>snapshots <input id="prunedays" class="input-smallint" type="number" min="0"> days, <input id="pruneweeks" class="input-smallint" type="number" min="0"> weeks and <input id="prunemonths" class="input-smallint" type="number" min="0"> months apart';
  html += '<h2>File uploads</h2>';
  html += '<p>Statistics: <span id="uploadstats">(loading)</span></p>';
  html += '<p><input type="checkbox" id="autodelete"> Delete unlinked file uploads after <input id="deleteafter" class="input-smallint" type="number" min="1" value="30"> days</p>';
  html += '<h2>Share to FlowNotes</h2>';
  html += '<p><span class="settings-label">Append-to note IDs:</span><input type="text" id="shareappend" placeholder="7,12,45" pattern="[0-9]+(, ?[0-9]+)*"></p>';
  html += '<h2>New note templates</h2>';
  html += '<p><span class="settings-label">Template note IDs:</span><input type="text" id="templatenotes" placeholder="2,8,16" pattern="[0-9]+(, ?[0-9]+)*"></p>';
  html += '<h2>Hashtag icons</h2>';
  html += '<p id="tag-icons"></p>';
  html += '<p><input type="button" id="addicon" class="modal-button-small" value="Add new hashtag icon"></p>';
  body.append(html);
